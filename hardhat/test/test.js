...
describe("Circuit test", function () {
  ...
  it("Transfer test", async () => {
    const circuit = await wasm_tester("circuits/transfer.circom");
    await circuit.loadConstraints();
...
describe("Verifier Transfer Contract", function () {
  let Verifier;
  let verifier;

  beforeEach(async function () {
    Verifier = await ethers.getContractFactory("Verifier_transfer");
    verifier = await Verifier.deploy();
    await verifier.deployed();
  });
  ...
  it("Should return true for correct proofs", async function () {
    const { proof, publicSignals } = await groth16.fullProve(
      ...
      "circuits/build/transfer_final.zkey"
    );
